...
private:
	...
	struct sNode
	{
		...
		bool bVisited = false;
		int nGlobalGoal; // possible distance to global goal
	...
	void AStarFind()
	{
		...
		for (int x = 0; x < nMapWidth; x++)
			for (int y = 0; y < nMapHeight; y++) {
				...
				nodes[y * nMapWidth + x].nGlobalGoal = abs(x - nodeEnd->x) + abs(y - nodeEnd->y);
				nodes[y * nMapWidth + x].nLocalGoal = int(1e9); // probably max distance possible

			}

		// heuristics for aStar algorithm
		auto aStarCompare = [&](pair <sNode*, int> lhs, pair <sNode*, int> rhs)
		{

			return lhs.first->nGlobalGoal + lhs.first->nLocalGoal >
				rhs.first->nGlobalGoal + rhs.first->nLocalGoal;

		};

		auto GetDist = [&](sNode* lhs) -> int // calculating manhattan distance to global goal
		{

			return lhs->nLocalGoal + lhs->nGlobalGoal;

		};

		priority_queue <pair <sNode*, int>, vector <pair <sNode*, int> >,
			decltype(aStarCompare)> pQ(aStarCompare); // way to use custom comparator in stl data-structures

		nodeStart->nLocalGoal = 0;

		pQ.push(make_pair(nodeStart, nodeStart->nGlobalGoal));
		...
		while (!pQ.empty()) {
			...
			if (curNode->nGlobalGoal + curNode->nLocalGoal > dist) // no possibility to do relaxation
				continue;
			...
			for (auto nextNode : curNode->vecNeighbours) {
				...
				if (!nextNode->bObstacle) {
					...
					if (GetDist(nextNode) > GetDist(curNode)) {
						nextNode->nLocalGoal = curNode->nLocalGoal + 1;
						nextNode->parent = curNode;
						pQ.push(make_pair(nextNode, GetDist(nextNode)));
...
protected:
	virtual bool OnUserCreate() override
	{
		...
		MainAlgo = &GraphAlgorithms::AStarFind;
	...
	virtual bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (currectPathPos == visitedNodes.size())
			if (nodeEnd->bVisited)
				currectPathPos--;
			else
				return true;
		...
		Sleep(15); // some extra delay so that user
				   // won't have epilepsy
